...
EXPERIMENT_NAME = 'NCF_dist_exp_2_embeddings_SWA'
import os
...
model = NCFDistribution.load_from_checkpoint(f'{DIR_RESULTS}/NCF_dist_exp_2_embeddings/NCF_dist_exp_2_embeddings_final_weights.ckpt', **params, **distance_params)
...
neptune_logger.experiment[f'ensemble/final_results'].upload(File(f'{DIR_RESULTS}/{EXPERIMENT_NAME}/{EXPERIMENT_NAME}_final_results.csv'))
neptune_logger.experiment[f'ensemble/final_weights'].upload(File(f'{DIR_RESULTS}/{EXPERIMENT_NAME}/{EXPERIMENT_NAME}_final_weights.ckpt'))
