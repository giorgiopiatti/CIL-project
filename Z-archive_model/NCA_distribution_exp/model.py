...
class Model(pl.LightningModule):
    ...
    def __init__(self, emb_size,  
        ...
        self.mha = MultiHeadAttention(number_of_movies, decoder_in_size, emb_size*num_heads, num_heads, 
                hidden_size_1, 
                hidden_size_2,
                device=self.device)
        self.alpha = alpha
        ...
        self.decoder = nn.Sequential(
            nn.Linear(decoder_in_size, decoder_in_size*2),
            nn.ReLU(),
            nn.Linear(decoder_in_size*2, decoder_in_size*4),
            nn.ReLU(),
            nn.Linear(decoder_in_size*4, number_of_movies*2)
        )

        self.dropout_decoder=nn.Dropout(p_dropout_decoder)


    def forward(self, batch):
        out = self.mha(batch)
        out = self.dropout_decoder(out)
