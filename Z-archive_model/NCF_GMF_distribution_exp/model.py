...
class NCFDistribution(pl.LightningModule):
    ...
    swa=False
        ...
        self.create_model(embedding_size, embedding_gmf_size, hidden_size, p_dropout, p_dropout_mlp)
    

    def create_model(self,embedding_size, embedding_gmf_size, hidden_size, p_dropout, p_dropout_mlp):
        ...
        self.embedding_gmf_users = nn.Embedding(number_of_users, embedding_gmf_size)
        self.embedding_gmf_movies = nn.Embedding(number_of_movies, embedding_gmf_size)
        
        self.mlp =  nn.Sequential(  
            nn.Linear(embedding_size*2, embedding_size),
            nn.Dropout(p_dropout_mlp),
            nn.ReLU(),
        ...
        self.prob_dist =  nn.Sequential(
            nn.Linear(embedding_gmf_size +embedding_size//4 , hidden_size),
    ...
    def forward(self, batch):
        ...
        concat_mlp = torch.cat([users_embedding, movies_embedding], dim=1)
        
        mlp = self.mlp(concat_mlp)
        embedding_gmf_users = self.embedding_gmf_users(users)
        embedding_gmf_movies = self.embedding_gmf_movies(movies)

        gmf = torch.mul(embedding_gmf_users, embedding_gmf_movies)

        concat = torch.cat([mlp, gmf], dim=1)
