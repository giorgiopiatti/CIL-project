...
ensemble_results_matrix_val = np.zeros((len(emb_sizes) * len(regs) * len(alphas), len(users_val)))
ensemble_results_matrix_test = np.zeros((len(emb_sizes) * len(regs) * len(alphas), len(users_test)))
...
for i, emb_size in enumerate(emb_sizes):
    for j, reg in enumerate(regs):
        for k, alpha in enumerate(alphas):
            ...
            ensemble_results_matrix_val[len(regs)*len(alphas)*i+len(alphas)*j + k] = val_res
            ensemble_results_matrix_test[len(regs)*len(alphas)*i+len(alphas)*j + k] = test_res
