...
def run_optuna(trial_fn, experiment_name, n_trials, n_gpus):
    ...
    with LocalCUDACluster(n_workers=n_gpus) as cluster, Client(cluster) as client:
        ...
        study.trials_dataframe().to_csv(
            f'{DIR_RESULTS}/{experiment_name}/{experiment_name}-optuna_results.csv')
        try:
            fig = optuna.visualization.plot_param_importances(study)
            fig.write_image(
                f'{DIR_RESULTS}/{experiment_name}/{experiment_name}-param_importances.png')
            fig = optuna.visualization.plot_optimization_history(study)
            fig.write_image(
                f'{DIR_RESULTS}/{experiment_name}/{experiment_name}-optimization_history.png')
        except:
            pass
